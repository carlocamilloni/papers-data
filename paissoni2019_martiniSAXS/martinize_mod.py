    ...
    if options["-nmap"].value:
        ...
        for i_count, i in enumerate(residues(atoms)):
            ...
            for j_count, j in enumerate(mapIndex(i)):
                ...
                for k in j:
                    if names[k[2]] == "O3'":
                        line += '%s,'%(str(o3_shift)) #my change 
                        line_mass += '%s,'%(str(k[0])) 
                        o3_shift = k[2]+atid
                    else:
                        line += '%i,'%(k[2]+atid) #my change
                        line_mass += '%s,'%(str(k[0]))
                    if k[2] + 1 > nra:
                        nra = k[2] + 1
                global_counter += 1
                line = 'bead' + str(global_counter) + ': CENTER ATOMS=' + line[:-1] + ' WEIGHTS=' + line_mass[:-1] + '\n'
                #nra += len(j)
                outNDX.write(line)
            atid += nra ##
